...
int ram_size_validation(int ram) {
    ...
    do {
        if (ram == 0) {
            cout<<"Enter RAM Space above 0 byte:"<<endl;
        }
        else {
...
int process_size_validation(int process_size) {
    ...
    do {
        if (process_size == 0) {
            cout<<"Enter Process size above 0 byte :"<<endl;
        }
        else {
            cout<<"You enter wrong size. Please try again."<<endl;
        }
...
int main() {
    ...
    cpu_demand =rand()%process_size;
    cout<<"CPU demands byte number:"<<cpu_demand<<endl;
