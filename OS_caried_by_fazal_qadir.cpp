...
struct Operating_System {
        ...
        struct Physical_Memory {
            ...
            Physical_Memory() {
                ...
                for (int i=0;i<Process.Number_of_Process;i++) {
                    ...
                    // Remaining_Time
                    Process.Process_Information[i].Process_Stats.Remaining_Time = Process.Process_Information[i].Process_Stats.Burst_Time;
                    // Completion Time
                ...
                for (int i=0;i<Process.Number_of_Process;i++) {
                    for (int j=i;j<Process.Number_of_Process;j++) {
                        ...
                        if (Process.Process_Information[i].Process_Stats.Arrival_Time>Process.Process_Information[j].Process_Stats.Arrival_Time){
                            ...
                            swap(Process.Process_Information[i].Process_Stats.Remaining_Time,Process.Process_Information[j].Process_Stats.Remaining_Time);
                            // swap Burst Time
    ...
    struct CPU_Management {
        ...
        static void node_deletion(Node *&Head,Node *&Tail) {
            Node *node = Head->next;
            if (node == nullptr) {
                Tail = nullptr;
            }
        ...
        struct Ready_Queue {
            ...
            Ready_Queue() {
                ...
                cout<<"*************************************************************\n";
                cout<<"Process PID "<<Physical_Memory.Process.Process_Information[0].Process_Stats.PID<<" enter ready Queue\n";
                cout<<"*************************************************************\n\n";
                node_insertion(Head,Tail,&Physical_Memory.Process.Process_Information[0].Process_Stats.PID);
            }
        };
        ...
        struct Round_Robin {
            ...
            Round_Robin() {
                ...
                while (Queue.Head != nullptr) {
                    ...
                    for (int j = 0; j < Queue.Number_of_Process; j++) {
                        if (*Queue.Head->Process_ID == Queue.Physical_Memory.Process.Process_Information[j].Process_Stats.PID) {
                    ...
                    Queue.Physical_Memory.Process.Process_Information[current_process_index].Process_Stats.Remaining_Time -= time_quantum;
                    cout<<"*************************************************************\n";
                    cout<<"Round-Robin select Process PID "<<Queue.Physical_Memory.Process.Process_Information[current_process_index].Process_Stats.PID<<"\n";
                    cout<<"*************************************************************\n\n";
                    CPU.Running_Process = std::to_string(Queue.Physical_Memory.Process.Process_Information[current_process_index].Process_Stats.PID);
                    CPU.Status = "BUSY";
                    // Ghant-Chart
                    if (!Ghant_Chart.empty()) {
                        Ghant_Chart+=" -> ";
                    }
                    Ghant_Chart += std::to_string(Queue.Physical_Memory.Process.Process_Information[current_process_index].Process_Stats.PID);
                    // How many Process arrives at time
                    while (i < Queue.Number_of_Process && Queue.Physical_Memory.Process.Process_Information[i].Process_Stats.Arrival_Time <= current_time) {
                        cout<<"*************************************************************\n";
                        cout<<"Process PID "<<Queue.Physical_Memory.Process.Process_Information[i].Process_Stats.PID<<" enter ready Queue\n";
                        cout<<"*************************************************************\n\n";
                    ...
                    // Display Output
                    display(CPU,Queue,time_quantum,Next_Process,Queue.Physical_Memory.Process.Process_Information[current_process_index].Process_Stats.Remaining_Time,Queue.Physical_Memory.Process.Process_Information[current_process_index].Process_Control_Block.state);
                    // if future process enter ready queue
                    if (Queue.Head == nullptr && i<Queue.Number_of_Process) {
                        // Increase Current time
                        current_time = Queue.Physical_Memory.Process.Process_Information[i].Process_Stats.Arrival_Time;
                ...
                // Display Ghant-Chart
                cout<<"*************************************************************\n";
                cout<<"Ghant-Chart: "<<Ghant_Chart<<endl;
                cout<<"*************************************************************\n\n";
            }
            ~Round_Robin() {
            ...
            static void display(struct CPU &CPU,Ready_Queue &Queue,int Time_Quanta,const string &Next_Process,const int &remian_time,string &state) {
                ...
                "------------------------------------------------------------\n";
                if (remian_time>0) {
                    cout<<"PID "<<CPU.Running_Process<<" time quantum expired.\n";
                    cout<<"PID "<<CPU.Running_Process<<" moved to the back of the Ready Queue.\n\n";
                    cout<<"Round Robin selected "<<*Queue.Head->Process_ID<<".\n"
                      "PID "<<*Queue.Head->Process_ID<<" dispatched to CPU Core 0.\n"
                      "------------------------------------------------------------\n\n";
                }
                else {
                    cout<<"PID "<<CPU.Running_Process<<" is successfully terminated\n"
                    "------------------------------------------------------------\n\n";
                }
            }
        ...
        struct  FCFS{
            ...
            void display(Node *&Curr,const struct CPU &cpu,const string &Next_process,const Ready_Queue &Q,int i) {
                ...
                if (Curr != nullptr) {
                    ...
                    cout<<"Process PID "<<*Curr->Process_ID<<" is executing CPU for "<<Queue.Physical_Memory.Process.Process_Information[i].Process_Stats.Burst_Time<<"sec\n";
                    while (true) {
                        auto current = std::chrono::steady_clock::now();

                        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(current-start).count();
                        if (elapsed >= Queue.Physical_Memory.Process.Process_Information[i].Process_Stats.Burst_Time) {
                            break;
                        }
                ...
                if (Curr) {
                    ...
                    Queue.Physical_Memory.Process.Process_Information[i].Process_Control_Block.state = "Terminated";
...
int main() {
    // In This Version I have used Round Robin Scheduling Algorithm
    // In version 2.1, I will add Banker's Algorithm.
    // In version 3.0, I will use Priority and Preemptive scheduling
    srand(time(nullptr));
    while (true) {
        if (Simulation_UI()) {
            Operating_System OS;
